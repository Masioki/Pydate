...
class user_dynamic_data(models.Model):
    userID = models.ForeignKey(user_static_data,on_delete=models.CASCADE)
...
class personal_question_user(models.Model):
    ...
    userID=models.ForeignKey(user_static_data,on_delete=models.CASCADE)          #ten co pyta

class personal_question_answer(models.Model):
    userID=models.ForeignKey(user_static_data,on_delete=models.CASCADE)          #ten co odpowiada
    questionID=models.ForeignKey(personal_question_content,on_delete=models.CASCADE)
...
class starter_question(models.Model):
    questionID= models.IntegerField(primary_key=True)
    content=models.CharField(max_length=250)
